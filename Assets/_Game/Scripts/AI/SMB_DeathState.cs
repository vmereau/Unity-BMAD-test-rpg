...
namespace Game.AI
{
    ...
    /// Calls AIAnimationDriver.EnableRagdoll() when the Death state exits (Death → Dead transition).
    /// OnStateExit fires exactly once at true animation end, not during crossfade.
    ///
    /// The Animator lives on a child GO (e.g. CreatureVisual); AIAnimationDriver lives on the root.
    /// GetComponentInParent walks up from the Animator's GO to find the driver on the root.
    /// </summary>
